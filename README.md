Tokenizer similar in structure to Byte Pair Encoding tokenizer (see  
...
Originally BPE was used at the level of words. But here it is used at the level of sentences. Text is splitted to original tokens -> words, and the words are merged to new 'idiomatic' tokens. 
The text James Joyse "Ulysses"" from the Gutemberg Project
http://www.gutenberg.org/ebooks/4300  is used as an example. 
...
Actually we may see the process of 'stop' words elimination. The words are not 'uselles' but their natural meaning arise is in combination with other words. The 'stop' words are resourses for 'idioms' and have to be used as the consolidated words combinations. Outside such combinations, the words may be really meaningless. 
My experience is that we must be very explicit with 'borders': the tokenization process is to be constrained by 'natural borders': border of words (if our tokens are chars and chars generated tokens) or by 'sentence' borders, if we run tokenizer at the level of words and words colections within 'sentences'. 

In the work the  text was split on sentences. But it is not enought: actual "borders" most probably correspond to punctuations. Sentences was additionally split on some punctuation marks like comma ',' (see the code for more details). 
...
Tokenizer will generate a new tokens from original words collections. The first token which is generated is: 
...
We may think about the tokens as (partly) 'text specific natural idioms'. Some idioms like 'of the' for sure just reflect the English text internal structure. It is quite natural, because the "Ulysses" is written in English. But some, like ' says the citizen',  are for sure very text specific. 

We may think also about the 'idioms' as a new (partly) text specific 'vocab'. Such idioms are to be considered as 'indivisible' pairs in text analysis, and may characterize a text much better , than just a single words. 'of the ' and 'of a' have very specific and quite clear meaning. But if we will take 'the',  'a' just as a separate words we will 
quickly come to the 'stop' words notion and the general receips in text analysis: the words is better to eliminate from specific text analyses, there are too many of such words, the words are spreaded within a text and adds nothing to a meaning.... etc.

The tokenization process may change the status of the words. If we will check the output of the programm, 
we will see in the initial vocabulary there are: ("the", 13676), ("of", 8171), ("and", 6695), ("a", 5885), ("to", 4873), ("in", 4726), ("his", 3063), ("he", 2880), ("I", 2683), ("that", 2415), ("with", 2414), ("it", 2145), ("was", 2094), 
("on", 2025), ("for", 1836), ("her", 1686), ("you", 1674), ("him", 1507), ("is", 1372), ("at", 1232)... where the second digit is quantity of the word in the text. 
...
And: 
1. The new tokens are much much more meaningfull and specific.  
...
Because the new tokens will be specific to the very text. 
2. In machine translation the idioms may/are to be translated 'as a whole' to similar idiom of target language. 'of the' has consolidated meaning and we may expect the presence of similar consolidated meaning - 'idiom' in a target language. In the Transformer like scheme, when we generate translation of target sentence word by word, basing on the previous word, starting from the first word, it may decrease the quantity of stages and increase quality of next word prediction. 
3. We may try to generate a 'natural basis' for machine translation and text analysis. 
Let us start from words and characters. We can split words to collection of characters, sentences to collection of characters too etc..... Every character will be basis vector in out system.   
...
Below is a sample output from the programm. The number of iteration is about 600. At about 550..600 iteration the quantity of newly generated tokens is about 10 only. The first iteration generate 'of the' 1640 pcs token. 
